  ...
  try {
    ...
    if (isZip) {
      ...
      data = {
        ...
        collections: meta.collections, files: meta.fileNames.map((name) => parseFile(decode(`${name}.sprite`), (kind, id) => entries[kind === 'frame' ? `${name}.sprite.frame-${id}` : `${name}.sprite.bin`] ?? null)),
      };
    } else {
