...
Sprite is a minimalist pixel-art editor: one modifier-driven tool, projects of multi-frame/multi-layer canvas files, autosave, no manual save.
...
**File**:
A single canvas document within a Project, with its own Layers, Frames, and undo history. Persisted as `.sprite`.
...
**Palette**:
Up to 256 color chips, belonging to a Project (not a File) — switching Files keeps it, switching Projects swaps it.
