# Sprite — Grilled Decisions

Every flagged assumption and open item from `INDEX.md`, `sprite-design-doc.md` §1, `sprite-features-doc.md` §12, and `sprite-ui-design-system.md` §8 has a confirmed answer below. This file is the resolution log for that grilling session. Where a spec file still says "flagged for confirmation" or leaves a value open, this file is the answer; where a later decision reversed one recorded here or in a spec file, the spec file has been amended in place and the reversal is recorded in an ADR under `docs/adr/`.
...
*(Reversed — a named global palette library, palette import and extraction are now in scope; see `todo/canvas-expansion-and-seven-features.md` B2a/B2b.)* Originally: user-facing palette swapping/customization beyond loading one of these three at Project creation was out of scope (YAGNI).
...
- First launch with no prior state creates one Project named `New Project` containing one File named `New Sprite` at 9×9 (the code's starter size; supersedes an earlier 16×16 note).
...
- **Accent red, exact:** `#BE1425`, sampled directly from `spec/stagwood.png`. Supersedes the `#C4202E` approximation used throughout both design docs' mockup language — treat every mention of `#C4202E` in `sprite-design-doc.md` / `sprite-ui-design-system.md` as superseded by this value.
...
- **Build phase order:** `sprite-design-doc.md` §19's 15 phases, followed as written — already dependency-ordered, no reprioritization.
